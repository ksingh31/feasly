...
describe('MockApiService', () => {
    ...
    describe('lead gate + magic link', () => {
        it('submit → dev token → verify round-trips', async () => {
            const preview = await firstValueFrom(service.getPreviewEstimate(estimateRequest));
            const lead = await firstValueFrom(service.submitLead({
    ...
    describe('report', () => {
        async function verifiedToken() {
            ...
            if (!verified.valid)
                throw new Error('mock verify failed');
        ...
        it('returns a snapshot with real ranges and the disclaimer footer', async () => {
            ...
            expect(report.narrative).toContain('Dollar figures are calculated deterministically from our cost model');
    ...
    describe('callbacks, sharing, analytics', () => {
        it('requestCallback echoes the window', async () => {
            const res = await firstValueFrom(service.requestCallback({
                reportToken: 'mock-x',
                name: 'Test Buyer',
                phone: '4035550100',
                window: 'morning',
        ...
        it('shareWithPartner echoes the recipient', async () => {
            const res = await firstValueFrom(service.shareWithPartner({ reportToken: 'mock-x', partnerEmail: 'partner@example.com' }));
            expect(res.sent).toBe(true);
