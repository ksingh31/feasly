 ...
 */
async function awaitSearchSettled(fixture) {
    ...
    for (;;) {
        ...
        if (component.status() !== 'searching')
            return;
        if (Date.now() > deadline)
            throw new Error('autocomplete did not settle');
        await new Promise((resolve) => setTimeout(resolve, 10));
    }
}
