 ...
 *
 * Sheets sync worker status (admin/05): health badge inputs (healthy /
 * lagging / failing / disabled), last run, pending leads, recent history.
 * Admin-only (X-Admin-Key interim until admin/01).
 *
 * Bundled by `npm run bundle:functions` into `sheets-status/index.js`
...
export async function sheetsStatusHandler(
  ...
  const result = await app.requestPipeline.run(
    ...
    () => app.sheetsStatusRoute.getStatus(headers),
  );
...
module.exports = sheetsStatusHandler;
