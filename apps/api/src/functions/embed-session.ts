/**
 * Azure Functions v3 trigger adapter — POST /api/v1/embed/session.
 *
 ...
 *
 * - Public endpoint: the iframe on a builder's page calls this
 *   unauthenticated to exchange its one-time relay code for a 12h session
 *   token. Abuse resistance comes from the standard public rate limiter on
 *   the request pipeline plus the code's own single-use + 10-minute-expiry
 *   semantics (embed/06).
 ...
 * - Bundled by `npm run bundle:functions` into `embed-session/index.js`
...
export async function embedSessionHandler(
  ...
  const result = await app.requestPipeline.run(
    ...
    () => app.embedSessionRoute.handle(req.body, clientIpFrom(req)),
  );
...
module.exports = embedSessionHandler;
