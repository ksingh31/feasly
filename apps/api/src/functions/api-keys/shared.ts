/**
 * Shared dispatch for the api-key admin Functions adapters (api-mcp/01).
 *
 * Follows the unsubscribe/shared.ts adapter pattern exactly: CORS is
 * enforced at the adapter edge (preflight never loads the composition), the
 ...
 * (currently the interim pre-shared-key guard; admin/01 swaps in session
 * auth). The guard runs INSIDE the pipeline so failures get the uniform
 * RFC 7807 envelope.
 */
...
export async function dispatchApiKeys(
