/**
 * Azure Functions v3 trigger adapter — POST /api/v1/estimates/{id}/narrative.
 *
 ...
 * - The per-estimate daily LLM cost guard lives in the narrative service,
 *   not here; the pipeline applies the general IP rate limit.
...
export async function narrativeHandler(
  ...
  const estimateId =
    typeof context.bindingData?.['id'] === 'string'
      ? (context.bindingData['id'] as string)
      : '';

  const result = await app.requestPipeline.run(
    { headers, clientIp: clientIpFrom(req) },
    () => app.narrativeRoute.generateNarrative(headers, estimateId),
  );
