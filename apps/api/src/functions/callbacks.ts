/**
 * Azure Functions v3 trigger adapter — POST /api/v1/callbacks.
 *
 * Same adapter pattern as `leads.ts`: CORS at the edge, correlation ID,
 * then the dedicated lead pipeline (10/min per IP — callbacks are
 * PII-carrying public mutations, same sensitivity class as the lead gate).
 * The request body carries the report token + name/phone/window; the token
 * is never logged.
 *
 * Bundled by `npm run bundle:functions` into `callbacks/index.js`
...
export async function callbacksHandler(
  ...
  // The body is never logged — it carries the report token (Bearer <redacted>)
  // plus callback PII (name/phone).
  const result = await app.leadPipeline.run(
    { headers, clientIp: clientIpFrom(req) },
    () => app.callbackRoute.handle(req.body),
  );
