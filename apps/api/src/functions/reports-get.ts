/**
 * Azure Functions v3 trigger adapter — GET /api/v1/reports/{reportToken}.
 *
 * Same adapter pattern as `magic-link-verify.ts`: CORS at the edge,
 * correlation ID, then the general request pipeline (100/min per IP —
 * frozen registry). The report token arrives as a path param; it is NEVER
 * logged (PII-grade Bearer <redacted>). A missing/empty token gets the same
 * denial shape as an unknown one — no oracle.
 *
 * Bundled by `npm run bundle:functions` into `reports-get/index.js`
...
export async function reportsGetHandler(
  ...
  const result = await app.requestPipeline.run(
    ...
    () => app.reportRoute.get(reportToken),
  );
...
module.exports = reportsGetHandler;
