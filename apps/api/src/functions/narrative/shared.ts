/**
 * Shared dispatch for the narrative Function adapter (consumer/06).
 *
 * Follows the privacy adapter pattern exactly: CORS is enforced at the
 * adapter edge (preflight never loads the composition), the composition is
 * built once per instance and cached module-level, and every response
 * carries the correlation ID. `invoke` is the route-specific service call.
 */
...
export async function dispatchNarrative(
  context: FunctionContext,
  req: FunctionRequest,
  invoke: (app: AppComposition) => Promise<unknown>,
): Promise<void> {
  // CORS is enforced at the adapter edge. The preflight path uses config
  // alone — the full composition (DB pool, rate limiters) is never loaded
